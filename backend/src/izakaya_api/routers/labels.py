...
def _create_pending_runs_for_type(db: Session, dataset_type: str) -> None:
    """Create pending runs for all DataSources of the given type."""
    ...
    for ds in sources:
        ...
        if not existing:
            db.add(PipelineRun(data_source_id=ds.id, status="pending"))
...
def label_summary(
    ...
    """Dashboard: per-dataset-type rule counts and column coverage."""
    ...
    col_counts = dict(
        db.query(LabelRule.dataset_type, func.count(func.distinct(LabelRule.column_name)))
        .group_by(LabelRule.dataset_type)
        .all()
    )
...
def bulk_save_rules(
    ...
    _create_pending_runs_for_type(db, dataset_type)
    db.commit()
...
def auto_label(
    ...
    col_result = _auto_label_column(db, dataset_type, dt, column_name, col_meta["description"])
    db.commit()
...
def auto_label_all(
    ...
    for col in string_cols:
        result = _auto_label_column(db, dataset_type, dt, col["name"], col["description"])
